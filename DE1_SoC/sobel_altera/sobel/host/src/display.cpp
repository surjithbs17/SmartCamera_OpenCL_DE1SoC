...
#define DUMP_IMAGE    "balloons_sobel_altera.ppm"
...
filter() {
  ...
  status = clEnqueueNDRangeKernel(queue, sobelKernel, 1, NULL, &sobelSize, &sobelSize, 0, NULL, NULL);
...
initCL() {
  ...
  status |= clSetKernelArg(sobelKernel, 2, sizeof(int), &pixels);  
  status |= clSetKernelArg(sobelKernel, 3, sizeof(unsigned int), &thresh);
