...
#define CL_FUNCTION "hough"
#define DEFAULT_IMAGE "balloons.ppm"
#define DUMP_IMAGE    "balloons_hough.ppm"
...
unsigned int hough_h, out_height, out_width;
...
cl_program program;
cl_kernel sobelKernel, houghKernel;
cl_mem in_buffer, out_buffer, hough_out_buffer;

size_t workSize[2];

void initCL();
void cleanup();
void teardown(int exit_status = EXIT_SUCCESS);
...
filter() {
  ...
  /* Sobel
  /***********************************************************************************************************************/    
  status = clEnqueueNDRangeKernel(queue, sobelKernel, 2, NULL, workSize, workSize, 0, NULL, NULL);
  ...
  /* Hough
  /***********************************************************************************************************************/
  status = clEnqueueNDRangeKernel(queue, houghKernel, 2, NULL, workSize, workSize, 0, NULL, NULL);
  checkError(status, "Error: could not enqueue Hough kernel");
  ...
  /* Read buffer
  /***********************************************************************************************************************/
  status = clEnqueueReadBuffer(queue, hough_out_buffer, CL_FALSE, 0, sizeof(unsigned int) * out_width * out_height, output, 0, NULL, NULL);
...
initCL() {
  ...
  sobelKernel = clCreateKernel(program, "sobel", &status);
  checkError(status, "Error: could not create sobel kernel");
      
  houghKernel = clCreateKernel(program, "hough", &status);
  checkError(status, "Error: could not create hough kernel");
    
  hough_h = (int) (sqrt(2.0) * width / 2);
	out_height = hough_h * 2;
	out_width = 180;
  ...
  out_buffer = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(unsigned int) * height * width, NULL, &status);
  checkError(status, "Error: could not create sobel out buffer");

  hough_out_buffer = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(unsigned int) * out_width * out_height, NULL, &status);
  checkError(status, "Error: could not create hough out buffer");

  // Sobel kernel - setting kernel arg
  status  = clSetKernelArg(sobelKernel, 0, sizeof(cl_mem), &in_buffer);  // frame in - image
  status |= clSetKernelArg(sobelKernel, 1, sizeof(cl_mem), &out_buffer); // frame out - sobel
  checkError(status, "Error: could not set sobel args");
  
  // Hough kernel - setting kernel arg
  status  = clSetKernelArg(houghKernel, 0, sizeof(cl_mem), &out_buffer); //frame in  -- sobel
  status |= clSetKernelArg(houghKernel, 1, sizeof(cl_mem), &hough_out_buffer); //frame out - hough
  status |= clSetKernelArg(houghKernel, 2, sizeof(cl_mem), &hough_h);
  checkError(status, "Error: could not set hough args");
  
  workSize[0] = width;   // Tell kernel the size of the image
  workSize[1] = height;  // ^
  
  // Create appropriate input/output buffers on the linux side
...
teardown(int exit_status) {
  ...
  if (hough_out_buffer) clReleaseMemObject(hough_out_buffer);
  if (sobelKernel) clReleaseKernel(sobelKernel);
  if (houghKernel) clReleaseKernel(houghKernel);
