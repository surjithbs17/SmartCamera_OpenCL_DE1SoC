...
#define CL_FUNCTION "sobel"
#define DEFAULT_IMAGE "balloons.ppm"
#define DUMP_IMAGE    "balloons_sobel_gray.ppm"
...
filter() {
  ...
  status = clEnqueueWriteBuffer(queue, in_buffer, CL_FALSE, 0, sizeof(unsigned int) * width * height, input, 0, NULL, NULL);
  ...
  checkError(status, "Error: could not enqueue sobel kernel");
...
initCL() {
  ...
  in_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(unsigned int) * width * height, NULL, &status);
  checkError(status, "Error: could not create device buffer");

  out_buffer = clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(unsigned int) * out_width * out_height, NULL, &status);
  checkError(status, "Error: could not create output buffer");

  status  = clSetKernelArg(sobelKernel, 0, sizeof(cl_mem), &in_buffer);
  status |= clSetKernelArg(sobelKernel, 1, sizeof(cl_mem), &out_buffer);
  ...
  input = (cl_uint*)alignedMalloc(sizeof(unsigned int) * width * height);
